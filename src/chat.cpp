...
const std::string PAGES_PATH = "../static/";
...
int main() {
	...
	CROW_ROUTE(app, "/")
		...
		return crow::mustache::load(PAGES_PATH + "sign_in_page.html").render();
	});
	...
	CROW_ROUTE(app, "/chat")
		...
		crow::response response;
		response.body = crow::mustache::load(PAGES_PATH + "chat.html").render();
	...
	CROW_ROUTE(app, "/chat/search/<string>")
		.methods("GET"_method)
		([&app](const crow::request& req, std::string query) {
		auto username = get_username(app, req);
		if (username == "") {
			return crow::response(401);
		}
	...
	CROW_ROUTE(app, "/chat/load_messages/<string>/<string>")
		.methods("GET"_method)
		([&app, &database_mutex, &res_mutex](const crow::request& req, crow::response& response, std::string friend_name, std::string str_time_stamp) {
		auto username = get_username(app, req);
		if (username == "") {
			response.code = 401;
			response.end();
	...
	CROW_ROUTE(app, "/chat/send")
		.methods("POST"_method)
		([&app, &start_time, &database_mutex, &res_mutex](const crow::request& req) {
		auto username = get_username(app, req);
		if (username == "") {
			return crow::response(401);
		}
		...
		auto time_stamp = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() - start_time;
		...
		if (wait_responses.find(username) != wait_responses.end()) {
			if (wait_responses[username].friend_name == friend_name) {
				...
				wait_responses[username].response->json_value["messages"][0]["message"] = message;
				wait_responses[username].response->end();
		...
		if (wait_responses.find(friend_name) != wait_responses.end()) {
			if (wait_responses[friend_name].friend_name == username) {
				...
				wait_responses[friend_name].response->json_value["messages"][0]["message"] = message;
				wait_responses[friend_name].response->end();
